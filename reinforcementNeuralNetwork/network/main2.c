...
int main() {
    ...
    if (load == 'y') {
        ...
        qNetwork = importNetworkJSON("qNetwork2.json");
        if (qNetwork == NULL) {
        ...
        targetNetwork = importNetworkJSON("targetNetwork2.json");
    ...
    for (int episode = 0; episode < EPISODES; episode++) {
        ...
        for (int moveCount = 0; moveCount < MAX_MOVES; moveCount++) {
            ...
            do {
                ...
                // NOTE: since this is agent 2, we swap network and opponent because I'm lazy & don't wanna change the rest of the code
                sscanf(buffer, "{%f,%f,%d,%d,%f,%f,%f,%f}", &nextState.oppPaddleY, &nextState.netPaddleY, &nextState.opponentPoints, &nextState.networkPoints, &nextState.ballX, &nextState.ballY, &nextState.ballVelX, &nextState.ballVelY);
    ...
    do {
        ...
        if (save == 'y') {
            ...
            exportNetworkJSON(qNetwork, "qNetwork2.json");
            printf("Q network saved\n");

            printf("Saving target network...\n");
            exportNetworkJSON(targetNetwork, "targetNetwork2.json");
...
// clear && clear && gcc main2.c -L/usr/local/lib -lneuralNetworkLib -I/usr/local/include -framework Accelerate -o main2 && ./main2

// or for debugging:
// clear && clear && gcc main2.c -L/usr/local/lib -lneuralNetworkLib -I/usr/local/include -g -framework Accelerate -o main2 && lldb ./main2